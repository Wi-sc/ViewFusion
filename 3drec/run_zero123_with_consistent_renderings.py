...
def run_anchor_view(device, folder_name, inference, models, scale=3.0, ddim_steps=75, ddim_eta=1.0, precision='fp32', h=256, w=256):
    ...
    anchor_view_num = 8
    anchor_rotation_cond = torch.zeros(anchor_view_num-1, 4)
    anchor_view_angle = torch.linspace(0, 2*np.pi-2*np.pi/anchor_view_num, steps=anchor_view_num)
    anchor_rotation_cond[:, 1] = torch.sin(anchor_view_angle[1:])
    anchor_rotation_cond[:, 2] = torch.cos(anchor_view_angle[1:])
    anchor_rotation_cond = anchor_rotation_cond.to(device)
    anchor_view_image = sample_model(inference.expand(anchor_view_num-1, -1, -1, -1).to(device), models, sampler, precision, ddim_steps, scale, ddim_eta, anchor_rotation_cond)
    

    multi_view_inference = torch.cat([inference, anchor_view_image*2-1], dim=0).to(device)
    if not os.path.exists(f"{folder_name}/anchor/"):
        os.makedirs(f"{folder_name}/anchor/")
    for i in range(multi_view_inference.shape[0]):
        # output_ims.append(Image.fromarray(x_sample.astype(np.uint8)))
        img = Image.fromarray((multi_view_inference[i].cpu().numpy().transpose(1,2,0)*127.5+127.5).astype(np.uint8))
        output_path = f"{folder_name}/anchor/{i%anchor_view_num}.png"
    ...
    x_samples_ddim_list = [inference/2+0.5]
    anchor_view_id = 1
    for view_id in range(degree_steps-1):
        rotation_cond_tmp = torch.zeros(2, 4)
        
        view_angle = 2*np.pi/degree_steps*(view_id+1)
        if anchor_view_id<anchor_view_num and view_angle>anchor_view_angle[anchor_view_id%anchor_view_num]:
            anchor_view_id += 1

        anchor_view_list = sorted([anchor_view_id-1, anchor_view_id%anchor_view_num])
        rotation_cond_angle = view_angle - anchor_view_angle[anchor_view_list]

        print(view_id, view_angle, rotation_cond_angle/np.pi*180, anchor_view_id, anchor_view_list)

        rotation_cond_tmp[:, 1] = torch.sin(rotation_cond_angle)
        rotation_cond_tmp[:, 2] = torch.cos(rotation_cond_angle)
        rotation_cond_tmp = rotation_cond_tmp.cuda()

        interpolation_weight = torch.nn.functional.softmax(rotation_cond_angle/(anchor_view_angle[1]-anchor_view_angle[0]))
        print(interpolation_weight)
        interpolation_weight = interpolation_weight.reshape(2, 1, 1, 1).to(device)
        x_samples_ddim_tmp = sample_model(multi_view_inference[anchor_view_list], models, sampler, precision, ddim_steps, scale, ddim_eta, rotation_cond_tmp, interpolation_weight=interpolation_weight)
        
        x_samples_ddim_list.append(x_samples_ddim_tmp.mean(dim=0, keepdim=True))
    x_samples_ddim = torch.cat(x_samples_ddim_list, dim=0)

    

    x_samples_ddim_for_save = 255.0 * rearrange(x_samples_ddim.cpu().numpy(), 'b c h w ->b h w c')
