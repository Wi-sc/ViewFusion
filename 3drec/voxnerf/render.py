...
def render_ray_bundle(model, ro, rd, t_min, t_max, return_density_loss=False):
    """
    ...
    weights, density_loss = volume_rend_weights(σ, step_size, return_density_loss=return_density_loss)
    ...
    if return_density_loss==False:
        return rgbs, E_dists, weights
    else:
        return rgbs, E_dists, weights, density_loss
...
def volume_rend_weights(σ, dist, return_density_loss=False):
    ...
    if return_density_loss:
        density_probability = (α*2).clamp(0.01, 1.)
        print("density_probability shape and statistic", density_probability.shape, density_probability.min(), density_probability.max())
        # density_probability = (α/torch.norm(α, p=1, dim=0, keepdim=True)).clamp(1e-6, 1)
        density_loss = -density_probability*torch.log(density_probability)
    else:
        density_loss = 0
    return weights, density_loss
