...
parser.add_argument("--output_dir", type=str, default="/home/ubuntu/data/abo/zero123_renderings")
parser.add_argument(
...
def randomize_camera():
    ...
    # return set_abo_camera_location(elevation, azimuth, distance)
    return set_get3d_camera_location(elevation, azimuth, distance)
...
def set_get3d_camera_location(elevation, azimuth, distance):
    radius = np.random.uniform(1.5, 2.2, 1)[0]
    x = radius*np.cos(azimuth/180*np.pi)*np.cos(elevation/180*np.pi)
    y = radius*np.sin(azimuth/180*np.pi)*np.cos(elevation/180*np.pi)
    z = radius*np.sin(elevation/180*np.pi)
...
def set_abo_camera_location(elevation, azimuth, distance):
    correct = False
    maxz=2.2
    minz=-2.2
    while not correct:
        radius = np.random.uniform(1.5, 2.2, 1)[0]
        x = radius*np.cos(1.5*np.pi)*np.cos(np.pi/6)
        y = radius*np.sin(1.5*np.pi)*np.cos(np.pi/6)
        z = radius*np.sin(np.pi/6)
        if maxz > z > minz:
            correct = True
...
if __name__ == "__main__":
    ...
    # args.output_dir = '/home/ubuntu/workspace/zero123/3drec/data/sofa_zero123_rendering_1'
    # args.num_images = 1
    ...
    #             break
    #     print(gt_mesh_path)
    #     save_images(gt_mesh_path)
    
    # args.output_dir = '/home/ubuntu/data/abo/zero123_rendering'
    # args.num_images = 20
    # data_dir = "/home/ubuntu/workspace/zero123/3drec/data/sofa"
    # gt_dir = "/home/ubuntu/data/abo/3dmodels/original"
    # model_name_list = os.listdir(data_dir)
