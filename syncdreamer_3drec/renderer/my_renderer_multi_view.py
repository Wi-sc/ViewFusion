...
class RendererTrainer(pl.LightningModule):
    ...
    def _init_dataset(self):
        ...
        model_name = self.image_path.split("/")[-2]
        print("=================="+model_name+"==================")
        ...
        print(model_dict[model_name][0], np.pi/2 - angle_json[str(model_dict[model_name][0])]["angle"][1], angle_json[str(model_dict[model_name][0])]["angle"][0], angle_json[str(model_dict[model_name][0])]["angle"][2])
        for cond_id in model_dict[model_name][1:]:
            rgb = imread(f'{self.image_path}/{cond_id}_condition.png')
            if self.use_mask:
                imsave(f'{self.log_dir}/input-{cond_id}_condition.png', rgb)
                masked_image = mask_predictor(rgb)
                imsave(f'{self.log_dir}/masked-{cond_id}_condition.png', masked_image)
                mask = masked_image[:,:,3].astype(np.float32)/255
            else:
                h, w, _ = rgb.shape
                mask = np.zeros([h,w], np.float32)
            ...
            e = np.pi/2 - angle_json[str(cond_id)]["angle"][1]
            distance = angle_json[str(cond_id)]["angle"][2]
            a = angle_json[str(cond_id)]["angle"][0] - angle_json[str(model_dict[model_name][0])]["angle"][0]
            print(cond_id, e, a, distance)
            row1 = np.asarray([np.sin(e)*np.cos(a),np.sin(e)*np.sin(a),-np.cos(e)])
            row0 = np.asarray([-np.sin(a),np.cos(a), 0])
            row2 = np.cross(row0, row1)
            R = np.stack([row0,row1,row2],0)
