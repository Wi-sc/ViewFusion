...
class GSODataset(Dataset):
    def __init__(self, generated_dir, save_dir='/home/ubuntu/data/abo/3dmodels/original'):
        self.meta = []
        self.mesh_scale = 0.8
        self.size = 128
        ...
        for model_name in model_dict:
            if model_name in ["B07R1JT4JL", "B07GFLD22Q"]:
                continue
            inference_id = model_dict[model_name]
            for folder_name in os.listdir(f"{save_dir}"):
                if os.path.exists(f"{save_dir}/{folder_name}/{model_name}.glb"):
                    gt_shape_path = f"{save_dir}/{folder_name}/{model_name}.glb"
            ...
            timestamp = os.listdir(f"{generated_dir}/{model_name}/")[0]
            gen_model_path = sorted(glob.glob(f"{generated_dir}/{model_name}/{timestamp}/plots/surface_*.ply"), key=lambda x: int(x.split("/")[-1].split("_")[1][:-4]))[-1]
            print(gen_model_path)
            meta_dict = {
                "model_name": model_name,
                "inference_index": inference_id,
                "gen_shape_path": gen_model_path,
                "gt_shape_path": gt_shape_path,
...
if __name__ == '__main__':
    ...
    processes = []
    output_dir = '/home/ubuntu/workspace/zero123/syncdreamer_3drec/multi_view/monosdf_abo'
