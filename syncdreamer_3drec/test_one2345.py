...
class GSODataset(Dataset):
    def __init__(self, generated_dir, save_dir='/home/ubuntu/data/GSO/shapes'):
        ...
        for model_name in model_dict:
            ...
            with open(f'/home/ubuntu/data/GSO/gso_renderings_elevation60/{model_name}/angles.json', "r") as f:
                angle_json = json.load(f)
            azimuth = -angle_json[str(inference_id)]['angle'][0] + 30/180*np.pi - np.pi/2
            # azimuth = 30/180*np.pi
            print(model_name, angle_json[str(inference_id)]['angle'][0]/np.pi*180, azimuth/np.pi*180)
            meta_dict = {
                "model_name": model_name,
                "inference_index": inference_id,
    ...
    def load_ply(self, shap_path, rotation_matrix):
        mesh_scale = 0.8
        mesh = trimesh.load(shap_path, force='mesh')
        mesh.vertices = mesh.vertices@rotation_matrix
        save_shape_path = shap_path[:-4]+"_canonical.obj"
        # if not os.path.exists(save_shape_path):
        # print(save_shape_path)
        mesh.export(save_shape_path)
    ...
    def __getitem__(self, idx):
        ...
        azimuth = meta_info['azimuth']
        rotation = np.array([[np.cos(azimuth), -np.sin(azimuth), 0],
                    [np.sin(azimuth), np.cos(azimuth), 0],
                    [0, 0, 1]])
        
        gen_mesh = self.load_ply(gen_shape_path, rotation)
...
if __name__ == '__main__':
    ...
    processes = []
    output_dir = '/home/ubuntu/workspace/zero123/syncdreamer_3drec/gso_one2345_estimate_elev'
