...
class Renderer(nn.Module):
    ...
    def forward(self, brdf_maps, rot_angle=None, light_shift=None, light_pos=None):
        ...
        if light_pos is not None:
            view_pos = torch.cat([xy_zeros, distance * z_ones], dim=1)
            light_pos = light_pos.unsqueeze(2).unsqueeze(3)
            light_pos = light_pos.expand_as(view_pos)
            center_light_dir = F.normalize(light_pos, p=2, dim=1)
        else:
            view_pos = torch.cat([xy_zeros, distance * z_ones], dim=1)
            light_pos = torch.cat([xy_zeros, distance * z_ones * 10], dim=1)
        ...
        shaded = shaded * (2.4 ** 2) * 100
        # print(shaded.shape, shaded.min(), shaded.max())
...
def ggx_microfacet(cos_half_angle, roughness):
    ...
    return alpha2 / (math.pi * sqr(sqr(cos_half_angle) * (alpha2 - 1.0) + 1.0) + 1e-6)
...
def get_smiths_shadowing(roughness, cos_view_angle, cos_light_angle):
    ...
    #             cos_view_angle * cos_view_angle + 1e-4)))
    # G1i = 2.0 / (1.0 + torch.sqrt(
    #     1.0 + alpha4 * (1.0 - cos_light_angle * cos_light_angle) / (
    #             cos_light_angle * cos_light_angle + 1e-4)))

    alpha2 = sqr(roughness)
    dv = torch.sqrt(1.0 + alpha2 * (1.0 - cos_view_angle * cos_view_angle) / (
                cos_view_angle * cos_view_angle + 1e-4))
    dl = torch.sqrt(1.0 + alpha2 * (1.0 - cos_light_angle * cos_light_angle) / (
                cos_light_angle * cos_light_angle + 1e-4))
    G1o = 2.0 / (1.0 + dv + 1e-4)
    G1i = 2.0 / (dl + dv + 1e-4)
    return G1o * G1i
...
def getMicrofacetReflectance(light_dir, view_dir, specular, roughness, normal):
    ...
    cos_light_angle = torch.clamp(cos_light_angle, 0.01, 0.99)

    cos_view_angle = cos_angle(view_dir, normal)
    cos_view_angle = torch.clamp(cos_view_angle, 0.01, 0.99)
    ...
    reflectance = reflectance / (4.0 * cos_light_angle * cos_view_angle + 1e-4) # bug!!!
