...
class RGB2BRDF(pl.LightningModule):
    ...
    def forward(self, input, sample_posterior=True):
        ...
        dec = torch.cat([albedo_maps, normal_maps, roughness_maps, specular_maps], dim=1)
    ...
    def training_step(self, batch, batch_idx):
        gt_rgb = self.get_input(batch, "rgb")
        gt_brdf_maps = self.get_input(batch, "brdf_maps")

        brdf_feature_maps, posterior = self(gt_rgb)
        albedo_maps = brdf_feature_maps[:, :3, ...]
        normal_maps = brdf_feature_maps[:, 3:6, ...]
        roughness_maps = brdf_feature_maps[:, 6:9, ...]
        specular_maps = brdf_feature_maps[:, 9:12, ...]
        brdf_maps = {
        ...
        optimizer_idx = None
        aeloss, log_dict_ae = self.loss(gt_rgb, rgb_reconstructions, posterior, self.global_step, optimizer_idx,
                                        last_layer=self.get_last_layer(), split="train")
        geo_loss = torch.abs(gt_brdf_maps-brdf_feature_maps)
        geo_loss = torch.sum(geo_loss) / geo_loss.shape[0]
        aeloss = aeloss + geo_loss

        self.log("train/aeloss_sum", aeloss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
        log_dict_ae["train/aeloss_geometry"] = geo_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        rgb_reconstructions = self.renderer(brdf_maps)
        rgb_reconstructions = rgb_reconstructions * 2 - 1
        aeloss, log_dict_ae = self.loss(gt_rgb, rgb_reconstructions, posterior, 0, self.global_step,
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        ...
        brdf_feature_maps, _ = self(gt_rgb)
        if not only_inputs:
            ...
            gt_albedo_maps = gt_brdf_maps[:, :3, ...]
            gt_normal_maps = gt_brdf_maps[:, 3:6, ...]
            gt_roughness_maps = gt_brdf_maps[:, 6:9, ...]
            gt_specular_maps = gt_brdf_maps[:, 9:12, ...]
