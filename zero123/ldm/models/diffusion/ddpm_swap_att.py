...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        T = rearrange(T, 'b v c h w -> (b v) c h w')
        ...
        if bs is not None:
            xc = xc[:bs]
        xc = rearrange(xc, 'b v c h w -> (b v) c h w')
        cond = {}
    ...
    def forward(self, x, c, cross_att=False, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, cross_att=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, cross_att=cross_att)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, cross_att=False):
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, cross_att=cross_att)
