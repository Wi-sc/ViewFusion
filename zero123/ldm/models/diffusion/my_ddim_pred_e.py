...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      t_start=-1, interpolation_weight=None):
        device = self.model.betas.device
        b = shape[0]
        if x_T is None:
            img = torch.randn([1, shape[1], shape[2], shape[3]], device=device)
        ...
        for i, step in enumerate(iterator):
            ...
            if interpolation_weight is not None:
                img = (img * interpolation_weight).sum(dim=0, keepdim=True)
                img = img.expand(shape)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      dynamic_threshold=None, interpolation_weight=None):
        ...
        noise = sigma_t * noise_like([1, *x.shape[1:]], device, repeat_noise) * temperature
