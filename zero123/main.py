...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 ...
                 log_images_kwargs=None, log_all_val=True):
        super().__init__()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            ...
            rank_zero_print(f"Attempting to resize cc_projection")
            input_weight = new_state["cc_projection.weight"]
            rank_zero_print(input_weight.shape)
            input_weight.zero_()
            input_weight[:, :772].copy_(old_state["cc_projection.weight"])
            old_state["cc_projection.weight"] = torch.nn.parameter.Parameter(input_weight)
        ...
        if 'metrics_over_trainsteps_checkpoint' in lightning_config:
            rank_zero_print(
